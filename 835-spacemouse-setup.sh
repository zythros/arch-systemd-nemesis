...
##################################################################################################################################
# Author    : zythros
# Purpose   : Install and configure spacenavd for 3Dconnexion SpaceNavigator on Arch Linux (systemd).
...
sudo tee /etc/systemd/system/spacenavd.service > /dev/null <<'UNITEOF'
[Unit]
...
tput setaf 2; echo "  /etc/systemd/system/spacenavd.service written."; tput sgr0
...
if [ -S /var/run/spnav.sock ]; then
    ...
    tput setaf 1
    echo "  WARNING: /var/run/spnav.sock not found — check: sudo systemctl status spacenavd" >&2
    tput sgr0
fi
