...
#             Skips automatically on bare metal
##################################################################################################################################
#
...
# systemd-detect-virt ships with systemd itself — no extra package needed here
# (artix-nemesis's OpenRC port had to pull in virt-what as a substitute).
...
sudo systemctl enable --now spice-vdagentd.service 2>/dev/null
sudo systemctl enable --now qemu-guest-agent.service 2>/dev/null
